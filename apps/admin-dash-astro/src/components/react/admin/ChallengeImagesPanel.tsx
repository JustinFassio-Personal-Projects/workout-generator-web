...
import type { ChallengeImageSlot } from '@/lib/challenge-image-slots';
import { CHALLENGE_IMAGE_SLOTS_WITH_LABELS } from '@/lib/challenge-image-slots';
...
const ChallengeImagesPanel: React.FC<ChallengeImagesPanelProps> = ({
  ...
  const handleRemove = async (slot: ChallengeImageSlot) => {
    ...
    try {
      ...
      toast.success(`${CHALLENGE_IMAGE_SLOTS_WITH_LABELS.find((s) => s.value === slot)?.label ?? slot} image removed`);
      onUpdate();
    } catch (err) {
    ...
    <>
      <div className="rounded-lg border border-white/10 bg-black/20 p-4">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
          {CHALLENGE_IMAGE_SLOTS_WITH_LABELS.map(({ value: slot, label }) => {
