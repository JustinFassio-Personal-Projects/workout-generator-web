...
import {
  CHALLENGE_IMAGE_SLOTS,
  type ChallengeImageSlot,
} from '@/lib/challenge-image-slots';
...
function isValidSlot(s: string): s is ChallengeImageSlot {
  return (CHALLENGE_IMAGE_SLOTS as readonly string[]).includes(s);
}

function buildPrompt(title: string, theme: string | undefined, slot: ChallengeImageSlot): string {
