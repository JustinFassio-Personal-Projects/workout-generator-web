...
/**
 * Fetch all workout sets for admin list. Returns [] only when the query succeeds with no rows.
  ...
  try {
    ...
    if (error) {
      if (import.meta.env.DEV || import.meta.env.PUBLIC_ENABLE_ERROR_LOGGING === 'true') {
        console.warn('[workout-sets] fetchWorkoutLibrary error:', error);
      }
      throw new Error(error.message || 'Failed to fetch workout library');
    }
