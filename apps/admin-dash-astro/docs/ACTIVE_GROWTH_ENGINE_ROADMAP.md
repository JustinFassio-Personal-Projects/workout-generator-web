...
**Companion work:** Instrumentation and funnels (e.g. hub monetization events in `analytics_funnel_events`) remain the **ground truth**; the Growth Engine **reads** those signals and proposes interventions.
...
- **Dashboard fatigue:** Teams scan many independent tables (`Monetization drop-off`, `Hub Activity`, retention cohorts, UTM breakdowns) without a single narrative.
...
- **Time to first key action** (from funnel / hub activity)
- **Hub activity** (frequency, recency, depth—e.g. generates vs completes)
...
- **Adoption:** 30-day (or 7-day) active usage of feature flags / events from **Hub Activity** (or equivalent event rollups).
...
|-------------|--------------|------------|
| Monetization drop-off | `analytics_funnel_events` (hub purchase funnel) | Revenue leak card, lead scoring (abandon recency) |
| Onboarding drop-off | `analytics_funnel_events` | Friction card, first-action timing |
| Hub activity | Hub activity rollup (existing admin query) | Feature matrix, friction narrative |
| Retention cohorts | Cohort queries | Messaging engine, trial urgency |
| Monetization candidates | Firebase-backed candidates API | Conversion pipeline, marketing card |
| Monetization (Phase 5) | Supabase profiles / program KPIs | Strategic context; avoid double-counting with hub funnel |
...
**Routing sketch:** `/admin/analytics/growth-engine` (hub page) + `/admin/analytics/details/:datasetKey` (or Astro dynamic routes under admin).
...
- **Source of truth:** Existing Supabase analytics tables + admin APIs (`/api/admin/analytics/*`) + Firebase-backed endpoints where already used (hub subscription state today is **Firebase/Firestore + Stripe**; admin monetization KPIs often read **Supabase `profiles`** — keep that split explicit when syncing **`growth_state`**, see §6 Prerequisite and §9).
...
3. **Firebase Cloud Function:** if you add a dedicated Firestore “checkout attempt” doc; otherwise prefer (1) keyed off Supabase funnel rows for one analytics brain.
...
- [x] Define the **canonical user growth state** as a **single mutually exclusive enum** at all times (extend only with care):
...
- [x] Document transitions (state diagram) and who owns each write path (hub client, Stripe webhook, scheduled reconciler).

### Phase A — Foundation (descriptive → diagnostic glue)

- [x] Add **Growth Engine** shell route + placeholder cards wired to **static copy**.
- [x] Inventory every **AnalyticsView** section → **datasetKey** + API + owner.
- [x] First **detail page** template (reusable layout: filters, glossary, export).
- [x] Phase A completed (2026-03-26).

### Phase B — Detail pages & APIs

- [x] Implement **detail routes** for top 3 datasets (Monetization drop-off, Hub activity, Retention).
- [x] Add **`/api/admin/growth-engine/context`** returning **structured JSON** for each dataset (for jobs and future LLM).
- [x] Add **`intervention_logs`** migration + **`POST /api/admin/growth-engine/interventions`** (admin-auth) for logging actions.

### Phase C — Rules-based Command Center (split batch + real-time)

- [x] **Batch:** Nightly (or hourly) job writes **daily_brief** record — Feature ROI inputs, cohort summaries, non-urgent Product/Engineering cards.
- [x] **Real-time / near-real-time:** Checkout-abandonment detector (§5.2) feeding **alert queue** or equivalent; surface on Growth Engine within minutes, not next morning.
- [x] Implement **rule pack v1** for the three alert cards, with **explicit** binding: which rules run in **batch** vs **realtime**.
...
- [x] Feature → event mapping table (config, not hardcoded).
- [x] Adoption rollups + **phase-1 correlation** heuristic.
- [x] Matrix viz + directive column.
- [x] Phase E completed (2026-03-26).

### Phase F — Messaging & experiments loop

- [x] Integrate UTM / landing / cohort summaries into suggestion queue.
- [x] “Propose experiment” flow (creates draft record; does not auto-launch).
- [x] Wire **intervention logging** into primary workflows (push/email/experiment launched) so §7 metrics are measurable.
- [x] Phase F completed (2026-03-26).
...
| Duplicate signals (hub vs Supabase monetization) | Document **which KPI answers which question**; label cards clearly |
...
**Checkout abandonment (real-time path)** maps to **`purchase_checkout_session_created`** in `analytics_funnel_events` vs downstream **`purchase_subscription_activated`** / **`purchase_return_success`** (see hub + astro-site instrumentation); the poller or edge job should use the same event names the **Monetization drop-off** detail page documents.

---

*Document status: **ACTIVE** — draft for engineering & growth alignment. Update phase checkboxes as work lands.*
