...
import {
  swapExerciseFlow,
  transformSwapOutputToFirestore,
  type AISwapRequestInput,
} from "@/lib/genkit/flows/swap-exercise";
import {
  estimateTokenUsage,
  estimateCostUsd,
...
/**
 * Zod-compatible schema for AISwapRequest (matches Genkit schema from swap-exercise.ts).
 * We need a separate Zod schema because Genkit uses its own z instance which is
 * incompatible with regular Zod schemas.
 */
const AISwapRequestSchemaZod = z.object({
...
const RequestBodySchema = z.object({
  ...
  swap_request: AISwapRequestSchemaZod,
});
  ...
  try {
    ...
    try {
      ...
      logger.error("Token verification failed", error, {
        route: "/api/workouts/ai-exercise-swap",
    ...
    const apiRateLimit = await checkApiRateLimit(uid, "ai_exercise_swap");
      ...
      return NextResponse.json(
        {
          ...
          message:
            "No active liability waiver is configured. Please contact support. AI exercise swapping is temporarily unavailable.",
        },
        { status: 503 }
      );
    }
    ...
    if (agreementSnapshot.empty) {
      logger.error(
        "[WAIVER CHECK FAILED] User attempted AI swap without agreeing to waiver",
        undefined,
        {
          route: "/api/workouts/ai-exercise-swap",
          operation: "waiver_check",
      ...
      return NextResponse.json(
        {
          ...
          message:
            "You must agree to the liability waiver before using AI exercise swapping. Please complete the waiver agreement and try again.",
    ...
    const { workout_id, section_index, exercise_index, swap_request } =
      validatedBody;
      ...
      return NextResponse.json(
        {
          ...
          message: buildLimitReachedMessage(rateLimit.tier, "AI swap"),
        },
    ...
    // 8. Validate and extract swap_request
    // Re-validate to ensure proper type inference
    const swapRequestValidation =
      AISwapRequestSchemaZod.safeParse(swap_request);
    if (!swapRequestValidation.success) {
      return NextResponse.json(
        {
          error: "Invalid swap_request",
          details: swapRequestValidation.error.flatten(),
        },
        { status: 400 }
      );
    }
    const validatedSwapRequest = swapRequestValidation.data;

    // 9. Build flow input
    const flowInput: AISwapRequestInput = {
      reason: validatedSwapRequest.reason,
      constraints: validatedSwapRequest.constraints,
      context: {
    ...
    // 10. Resolve prompt overrides for AI swap (admin-managed)
    const trainerDoc =
    ...
    // 11. Call Genkit flow
    const aiOutput = await swapExerciseFlow(flowInput, promptOverrides);

    // 12. Transform output to Firestore format
    const transformed = transformSwapOutputToFirestore(aiOutput);
    ...
    await usageLogRef.set({
      ...
      edit_type: "ai_swap",
      // edit_mode is not applicable for swaps
      user_prompt: validatedSwapRequest.reason.substring(0, 500), // Truncate for logs
    ...
    incrementMetric("exercise.swapped", 1, { tier: rateLimit.tier });
    return NextResponse.json({
      success: true,
    ...
    captureApiError(error, {
      endpoint: "ai_exercise_swap",
      operation: "ai_exercise_swap",
    });
    incrementMetric("ai.failure", 1, { endpoint: "ai_exercise_swap" });

    logger.error("AI exercise swap error", error, {
      route: "/api/workouts/ai-exercise-swap",
      operation: "ai_exercise_swap",
    });
