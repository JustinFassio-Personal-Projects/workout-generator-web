      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {results.map((result, index) => (
          <a
            ...
            className="group relative flex flex-col p-5 bg-white dark:bg-brand-dark/60 border border-slate-200 dark:border-white/5 rounded-xl hover:border-brand-green/30 hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-all duration-300 overflow-hidden shadow-sm hover:shadow-md"
          >
            ...
            <div className="flex items-start justify-between gap-3 mb-3">
              <h4 className="font-display font-bold text-slate-800 dark:text-slate-200 group-hover:text-brand-green transition-colors line-clamp-2 leading-tight text-sm">
                {result.title}
              </h4>
              <ExternalLink className="w-3.5 h-3.5 text-slate-400 dark:text-slate-600 group-hover:text-brand-green flex-shrink-0 transition-colors mt-0.5" />
            </div>
