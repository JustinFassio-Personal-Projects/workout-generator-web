...
export const OnboardingWizard: React.FC<OnboardingWizardProps> = ({ preselect, tenantId }) => {
  ...
  const validateStepTwo = useCallback((): boolean => {
    ...
    if (
      ...
      (!Number.isInteger(formData.age) || formData.age < 13 || formData.age > 120)
    ) {
  ...
  const handleUnitsChange = useCallback((units: Partial<PreferredUnits>) => {
    setFormData(prev => ({ ...prev, preferred_units: { ...prev.preferred_units, ...units } }))
  }, [])
  ...
  const handleCreateAccount = useCallback(() => {
    ...
    if (typeof window !== 'undefined' && window.posthog) {
      window.posthog.capture('onboarding_create_account_clicked', {
        fitness_goals: formData.fitness_goals,
        fitness_level: formData.fitness_level,
        equipment_access: formData.equipment_access,
        ...
        <div className="max-w-6xl mx-auto transition-all duration-500 min-h-[50vh] md:min-h-[70vh] flex flex-col justify-center">
          ...
          >
            <div className="relative group">
              <div className="absolute -inset-1 bg-gradient-to-r from-brand-green via-brand-lime to-emerald-500 rounded-3xl opacity-20 group-hover:opacity-40 transition duration-500 blur-xl" />
