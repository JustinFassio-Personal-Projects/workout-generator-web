...
import styles from './OnboardingIntroScreen.module.scss'

interface OnboardingIntroScreenProps {
  /** When false: onStart() is called on CTA click (used inside OnboardingWizard). When true: navigate to /onboard (used on landing page). */
  onStart?: () => void
  /** When true, "Generate Workout" and "Free Workout" navigate to /onboard instead of calling onStart. */
  standalone?: boolean
}

export function OnboardingIntroScreen({ onStart, standalone = false }: OnboardingIntroScreenProps) {
  const handleStart = () => {
    if (standalone) {
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={`${styles.bottomContent} ${phase === 3 ? styles.visible : styles.hidden}`}>
        ...
        <div className={styles.ctaButtonGroup}>
          <button
            ...
            data-analytics="onboard_intro_generate_workout"
          >
      ...
      <button
        ...
        data-analytics="onboard_intro_free_workout"
      >
        Free Workout
      </button>
    </div>
