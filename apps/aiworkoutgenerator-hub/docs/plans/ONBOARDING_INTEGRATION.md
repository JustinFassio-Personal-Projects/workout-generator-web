...
function validateEquipmentAccess(
  ...
  return equipmentAccess.every((category) =>
...
function normalizeOnboardingData(urlParams: URLSearchParams) {
  const fitnessLevel =
    (urlParams.get("fitness_level") as FitnessLevel) || DEFAULT_FITNESS_LEVEL;
  ...
  const allowedCategories = categoryMapping[fitnessLevel];
  const validEquipmentAccess = equipmentAccess.filter((cat) =>
    allowedCategories.includes(cat)
  );

  // Ensure at least one category
  const normalizedEquipmentAccess =
    validEquipmentAccess.length > 0
      ? validEquipmentAccess
