...
interface ExerciseCardPlayerProps {
  ...
  /** When set with onUpdateSet, Set Details uses the shared log (weight, per-set done, complete exercise). */
  sectionIdx?: number;
  exerciseIdx?: number;
  onUpdateSet?: (
    sectionIdx: number,
...
export function ExerciseCardPlayer({
  ...
  const interactiveSetLog =
    onUpdateSet != null && sectionIdx !== undefined && exerciseIdx !== undefined
      ? {
          sectionIdx,
          exerciseIdx,
          onUpdateSet,
      ...
      >
        {isActive ? (
          <ResizableExerciseSplit
            storageKey="exercise-card-player-active"
            defaultLeftPercent={50}
            minLeftPercent={26}
            maxLeftPercent={76}
            rowClassName="md:min-h-[600px]"
            left={
              <div className="w-full min-w-0 overflow-y-auto flex flex-col md:h-full">
                {/* Header with badges */}
                      ...
                      <AccordionItem value="details" className="border-none">
                        ...
                        <AccordionContent className="pt-2">
                          <ExerciseDetails
                            ...
                            safetyMode={false}
                            isActive={isActive}
                            interactiveSetLog={interactiveSetLog}
                          />
                        </AccordionContent>
                      </AccordionItem>
                    </Accordion>
            ...
            right={
              <div className="w-full min-w-0 shrink-0 bg-muted flex items-stretch md:h-full">
...
function ExerciseDetails({
  ...
  safetyMode,
  isActive = false,
  interactiveSetLog = null,
}: {
  exercise: TrainerWorkoutExercise;
  remainingCues: string[];
  safetyMode: boolean;
  isActive?: boolean;
  interactiveSetLog?: InteractiveSetLogProps | null;
}) {
  return (
        ...
        <div>
          ...
          {interactiveSetLog ? (
            <div className="border border-border rounded-lg overflow-hidden p-2 sm:p-3">
              <ExerciseSetLogTable
                exercise={exercise}
                sectionIdx={interactiveSetLog.sectionIdx}
                exerciseIdx={interactiveSetLog.exerciseIdx}
                onUpdateSet={interactiveSetLog.onUpdateSet}
                onToggleSetComplete={interactiveSetLog.onToggleSetComplete}
                showLoggingColumns
              />
              <ExerciseCompleteFooter
                exercise={exercise}
                sectionIdx={interactiveSetLog.sectionIdx}
                exerciseIdx={interactiveSetLog.exerciseIdx}
                onExerciseComplete={interactiveSetLog.onExerciseComplete}
