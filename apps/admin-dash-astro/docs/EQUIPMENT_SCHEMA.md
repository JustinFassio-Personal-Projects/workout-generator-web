...
**CLI (`supabase db push`):** Credentials live in repo root **`.env.local`** (gitignored): `SUPABASE_ACCESS_TOKEN` and `SUPABASE_DB_PASSWORD`. From repo root, load them then run: `set -a && source .env.local && set +a && supabase link --project-ref <PROJECT_REF> && supabase db push`. Replace `<PROJECT_REF>` with your project ref (Dashboard URL or Project Settings → General). If you don’t have `.env.local`, copy `.env.example` to `.env.local` and fill in (token from Account → Access Tokens; DB password from Dashboard → Project Settings → Database).
