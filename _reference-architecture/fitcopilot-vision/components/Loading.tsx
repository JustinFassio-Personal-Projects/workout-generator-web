...
const Loading: React.FC<LoadingProps> = ({ status, step, facts = [] }) => {
      ...
      >
        {type === 'icon'
          ? React.createElement(content, {
              className: 'w-5 h-5 md:w-6 md:h-6 filter drop-shadow-[0_0_8px_rgba(132,204,22,0.5)]',
            })
          : content}
      </div>
    )
  ...
  return (
    <div className="relative flex flex-col items-center justify-center w-full max-w-4xl mx-auto mt-8 min-h-[350px] md:min-h-[500px] overflow-hidden rounded-3xl bg-white/40 dark:bg-brand-dark/40 border border-slate-200 dark:border-white/10 shadow-2xl backdrop-blur-md transition-colors">
      <style>{`
        @keyframes implode {
      ...
      <div className="relative z-20 mb-10 md:mb-16 scale-[0.65] md:scale-125 mt-4 md:mt-10">
        <div className="absolute inset-0 w-64 h-64 -translate-x-[4.5rem] -translate-y-[4.5rem] border border-dashed border-brand-green/30 rounded-full animate-[spin-slow_20s_linear_infinite]"></div>
        ...
        <div className="relative bg-white/50 dark:bg-white/10 p-1 rounded-full shadow-[0_0_60px_rgba(34,197,94,0.4)] animate-[pulse-core_2s_infinite]">
          <div className="bg-white dark:bg-brand-dark p-4 rounded-full flex items-center justify-center w-24 h-24 relative overflow-hidden border border-brand-green/50">
            ...
            <BrainCircuit className="w-12 h-12 text-brand-green animate-pulse relative z-10" />
        ...
        <div className="absolute top-1/2 left-1/2 w-[300px] md:w-[500px] h-[300px] md:h-[500px] -translate-x-1/2 -translate-y-1/2 pointer-events-none">
          ...
          <FlyingItem content="FORM" type="text" delay={0.7} position={4} />
          <FlyingItem content={Dna} type="icon" delay={1.0} position={5} />
          <FlyingItem content="CARDIO" type="text" delay={1.2} position={6} />
          <FlyingItem content={Globe} type="icon" delay={1.5} position={7} />
          <FlyingItem content="RECOVERY" type="text" delay={1.7} position={8} />
      ...
      <div className="relative z-30 w-full max-w-lg bg-white/80 dark:bg-brand-dark/80 backdrop-blur-xl rounded-2xl p-6 md:p-8 shadow-2xl border border-slate-200 dark:border-white/10 text-center flex flex-col items-center transition-all duration-500 min-h-[140px] md:min-h-[160px]">
        <div className="flex items-center gap-3 mb-4">
          ...
          {step === 2 && <Atom className="w-4 h-4 text-brand-green animate-spin" />}
          {step >= 3 && <Microscope className="w-4 h-4 text-emerald-500 animate-bounce" />}
          <h3 className="text-brand-green font-bold text-[10px] md:text-xs tracking-[0.2em] uppercase font-display">
            {status}
          </h3>
        </div>
