...
function equipmentArrayToAccess(arr: string[]): EquipmentAccess {
...
/**
 * Parse onboarding data from URL search params (same param names as buildSignupUrl).
 * Used to pre-fill the builder and to persist form state in the URL.
 */
...
/**
 * Build URLSearchParams from onboarding data (same as buildSignupUrl but returns params only).
 * Used to sync form state to the URL without navigating.
 */
