...
export function notifyMainSiteRevalidate(): void {
  const url =
    (typeof process !== 'undefined' && process.env?.MAIN_SITE_REVALIDATE_URL) ||
    (import.meta as { env?: Record<string, string> }).env?.MAIN_SITE_REVALIDATE_URL;
  if (!url || typeof url !== 'string' || url.trim() === '') return;

  const secret =
    (typeof process !== 'undefined' && process.env?.MAIN_SITE_REVALIDATE_SECRET) ||
    (import.meta as { env?: Record<string, string> }).env?.MAIN_SITE_REVALIDATE_SECRET;
  ...
  void fetch(url.trim(), {
    method: 'POST',
    headers,
    body: JSON.stringify({ source: 'admin-dash-astro' }),
  }).catch(() => {
